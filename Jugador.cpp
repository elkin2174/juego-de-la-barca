...
{
  // Crea los lugares
  addLugar(new Orilla("OrillaIzquierda"));
  addLugar(new Barca("Barca"));
  addLugar(new Orilla("OrillaDerecha"));
  ...
  lugares[2]->setVecinoAlternativo(lugares[1]);// Vaciono Alternativo: BArca
  //agrega los individuos a el lugar inicial;
  lugares[0]->addIndividuos(new Individuo("Robot","NOCome",'R'));
...
void Jugador::jugar()
{
  ...
  while(true)
  {
    ...
    if(ordenMenu != 'B')
    {
      if (ordenMenu == 'Q' or validarPerdida(ordenMenu))
      {
      ...
      {
        cout<<"\t ¡La Barca Esta llena!"<<endl<<endl;
        continue;
      }
      if(ordenMenu == 'R' or ordenMenu == 'Z' or ordenMenu == 'C' or ordenMenu == 'L')
      {
      ...
      {
        cout<<" Por favor, Ingresa una accion valida. "<<endl;
      }
    
      if (validarPerdida(ordenMenu))
      {
    ...
    else if(ordenMenu == 'B')
    {
      ...
      else cout << "\t ¡El robot no se ecuentra en la barca!\n"<<endl<<endl; 
    }
  }
}
...
bool Jugador::validarPerdida(char orden)
{
  for(Lugar *lugar : lugares)
  {
    ...
    if(lugar->seAhogo(lugar->vectorIndividuos()[lugar->retornarIndice(orden)]))
    {
      return true;
    }
...
{
  if(lugares[2]->vectorIndividuos().size() == 4)
  {
